...
CREATE OR REPLACE FUNCTION public.osgr_find_datum(gridref TEXT)
    RETURNS INT
    LANGUAGE plpgsql
AS $$
...
END;$$;-- Finds the easting of a provided Channel Islands grid reference in datum units (m)

CREATE OR REPLACE FUNCTION public.osgr_ci_easting(s1 TEXT)
    RETURNS INT
    LANGUAGE plpgsql
AS $$
...
BEGIN
    letters = (regexp_matches(Upper(s1), '^([A-H|J-Z]{2}(?:[0-9][0-9])+)(?:(?:[NS][EW])|[A-D])?$'))[1];
    IF letters IS NULL THEN
        RAISE EXCEPTION 'Invalid gridref supplied: %', s1;
    END IF;
    letters = (regexp_matches(letters, '[A-Z]+'))[1];
    numbers = (regexp_matches(s1, '[0-9]+'))[1];
...
CREATE OR REPLACE FUNCTION public.osgr_ci_northing(s1 TEXT)
    RETURNS INT
    LANGUAGE plpgsql
AS $$
...
CREATE OR REPLACE FUNCTION public.osgr_gb_easting(s1 TEXT)
    RETURNS INT
    LANGUAGE plpgsql
AS $$
...
CREATE OR REPLACE FUNCTION public.osgr_gb_northing(s1 TEXT)
    RETURNS INT
    LANGUAGE plpgsql
AS $$
...
CREATE OR REPLACE FUNCTION public.osgr_ing_northing(s1 TEXT)
    RETURNS INT
    LANGUAGE plpgsql
AS $$
...
BEGIN
    letters = (regexp_matches(Upper(s1), '^([A-H|J-Z]{1}(?:[0-9][0-9])+)(?:(?:[NS][EW])|[A-D])?$'))[1];
    IF letters IS NULL THEN
        RAISE EXCEPTION 'Invalid gridref supplied: %', s1;
    END IF;
    letters = (regexp_matches(letters, '[A-Z]+'))[1];
...
CREATE OR REPLACE FUNCTION public.osgr_ing_easting(s1 TEXT)
    RETURNS INT
    LANGUAGE plpgsql
AS $$
...
BEGIN
    ...
    RETURN letterConverted||numPad;
END;$$;-- A handler function that returns the easting of a provided grid ref in the determined datum (see find_datum for more details)
...
CREATE OR REPLACE FUNCTION public.osgr_to_ci(easting INT, northing INT, accuracy INT)
    RETURNS TEXT
    LANGUAGE plpgsql
AS $$
...
CREATE OR REPLACE FUNCTION public.osgr_to_gb(easting INT, northing INT, accuracy INT)
    RETURNS TEXT
    LANGUAGE plpgsql
AS $$
...
CREATE OR REPLACE FUNCTION public.osgr_to_ing(easting INTEGER, northing INTEGER, accuracy INTEGER)
    RETURNS TEXT
    LANGUAGE plpgsql
AS $$
...
CREATE OR REPLACE FUNCTION public.osgr_to_gridref(easting INT, northing INT, accuracy INT, datum INT)
    RETURNS TEXT
    LANGUAGE plpgsql
AS $$
BEGIN
    CASE(datum)
        WHEN 29901 THEN
            RETURN public.OSGR_TO_ING(easting, northing, accuracy);
        WHEN 27700 THEN
            RETURN public.OSGR_TO_GB(easting, northing, accuracy);
        WHEN 32630 THEN
            RETURN public.OSGR_TO_CI(easting, northing, accuracy);
        ELSE
